...
class StringIteratorIO:
    ...
    def __init__(self, path, encoding=None):
        ...
        if (encoding is not None) and encoding.startswith('auto'):
            ...
            if len(encoding) > 1:
                ...
                n_bytes = 5000
            with self._open_binary() as file:
    ...
    def _open_binary(self):
        if str(self._path).lower().endswith('.gz'):
            return gzip.open(self._path, 'rb')
        return open(self._path, 'rb') # pylint: disable=consider-using-with

    def _open_text(self, encoding):
        if str(self._path).lower().endswith('.gz'):
            return gzip.open(self._path, 'rt', encoding=encoding)
        return open(self._path, 'r', encoding=encoding) # pylint: disable=consider-using-with
    ...
    def __enter__(self):
        self._f = self._open_text(self._encoding)
        return self
...
def _get_path(line, data_dir, logger, raise_errors):
    ...
    try:
        ...
        gz_look_path = Path(str(look_path) + '.gz')
        try:
            actual_path = case_insensitive_path(gz_look_path)
...
def read_restartdate_from_buffer(buffer, attr, logger):
    ...
    full = None
    for line in buffer:
        has_terminator = line.rstrip().endswith('/')
        content = line.rsplit('/', 1)[0].strip() if has_terminator else line.strip()
        if not content:
            break
        vals = content.split()[:len(columns)]
