...
class Wells(BaseTree):
    ...
    def add_welltrack(self, segment):
        ...
        if 'WELSPECS' in segment.attributes and not segment.WELSPECS.empty:
            i0, j0 = segment.WELSPECS[['I', 'J']].values[0]
        else:
            start_ij = df[['I', 'J']].replace({None: np.nan}).replace(INT_NAN, np.nan).dropna().head(1)
            if start_ij.empty:
                return self
            i0, j0 = start_ij.values[0]
    ...
    def fill_na(self, segment, attr):
        ...
        if attr in segment.attributes:
            ...
            if set(('I', 'J')).issubset(set(data.columns)):
                if 'WELSPECS' not in segment.attributes or segment.welspecs.empty:
                    return self
